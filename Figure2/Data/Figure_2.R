...
library(readr)
library(dplyr)
df <- read_csv("FCS_WES.csv")
df <- df %>%
  filter(!is.na(Final.label)) %>%
  mutate(
    Final.label = factor(Final.label, levels = c("ATM", "TP53", "WT")),
    ATM.final.status = factor(ATM.final.status)
  )
a<-ggplot(df, aes(x = FCS, y = Final.label)) +
  geom_violin(fill = "gray90", color = "black", scale = "width", trim = FALSE) +
  geom_jitter(aes(color = ATM.final.status), width = 0.3, size = 2, alpha = 0.8) +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal(base_size = 14) +
  labs(title = "FCS",
       x = "FCS", y = NULL, color = "ATM Status") +
  theme(
...
b<-ggplot(df, aes(x = FCS, y = Final.label)) +
  geom_violin(fill = "gray90", color = "black", scale = "width", trim = FALSE) +
  geom_jitter(aes(color = ATM.final.status), width = 0.3, size = 2, alpha = 0.8) +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal(base_size = 14) +
  labs(title = "BCS",
       x = "BCS", y = NULL, color = "ATM Status") +
  theme(
    panel.grid = element_blank(),
    axis.text.y = element_text(face = "italic"),
    plot.title = element_text(face = "bold")
...
c<-ggplot(df, aes(x = GCS, y = Final.label)) +
  geom_violin(fill = "gray90", color = "black", scale = "width", trim = FALSE) +
  geom_jitter(aes(color = ATM.final.status), width = 0.3, size = 2, alpha = 0.8) +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal(base_size = 14) +
  labs(title = "GCS",
       x = "GCS", y = NULL, color = "ATM Status") +
  theme(
    panel.grid = element_blank(),
    axis.text.y = element_text(face = "italic"),
    plot.title = element_text(face = "bold")
  )
pdf("Coh-1.pdf", height=10, width=10)
